...
int main(int argc, char *argv[])
{
	...
	/***** Implement OpenMP HERE *****/
	#pragma omp parallel for private(hash,i,j) shared(wl,num_hf,wl_size) schedule (guided, 512)
	for (i = 0; i < wl_size; i++) {
		for (j = 0; j < num_hf; j++) {
	...
	misspelled = 0;
	/***** Implement OpenMP HERE *****/
	#pragma omp parallel for private (hash,j) shared(word) schedule (guided, 512)
	for (j = 0; j < num_hf; j++) {
		hash = hf[j] (word);
		hash %= bv_size;
		if (bv[hash] == 0)
