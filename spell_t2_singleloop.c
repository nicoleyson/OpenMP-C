...
int main(int argc, char *argv[])
{
	...
	/***** Implement OpenMP HERE (Option 1)*****/
	#pragma omp parallel for private(hash,j) shared(wl,num_hf,wl_size) schedule (guided, 256)
	for (i = 0; i < wl_size; i++) {
		for (j = 0; j < num_hf; j++) {
	...
	/***** Implement OpenMP HERE (Option 2) *****/
/*	#pragma omp parallel for private (hash,j) shared(word) */
	for (j = 0; j < num_hf; j++) {
		hash = hf[j] (word);
		hash %= bv_size;
		if (bv[hash] == 0)
